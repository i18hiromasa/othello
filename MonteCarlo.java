public class MonteCarlo{
    ...
    public static int NODE_MAX = 15000;//Nodeを最大で作れる数
    public static Node node[] = new Node[NODE_MAX];//Nodeを保持
    public static int nodeNum = 0;//Nodeを作った数
    public static int playout(Othello board){//playoutを行うメソッド
        allPlayouts++;
    ...
    public static int searchUct(Othello board, int num){//UCBに基づいて手を探すメソッド
        Node n = node[num];
        int select = -1;
        ...
        for(int i = 0; i < n.getChildNum(); i++){
            ...
            int x = xy / 10;
            int y = xy % 10;
            if(board.getBoard(x,y) != Othello.CAN_PUT_STONE)continue;
            double ucb = 0;
            if(c.getGames() == 0){
    ...
    public static int selectBestUct(Othello board){//選ばれた手の中で一番選ばれた回数が多い手を返すメソッド
        nodeNum = 0;
        int next = createNode(board);
        int uctLoop = 9000;
        Othello copy = new Othello();
        board.copy(copy);
    ...
    public static int createNode(Othello board){//Nodeを作るメソッド
        Node n = new Node();
        for(int i = 1; i < 9; i++){
            for(int j = 1; j < 9; j++){
                if(board.getBoard(i, j) == Othello.CAN_PUT_STONE){
                    int xy = (i * 10) + j;
    ...
    public static int searchUct(Othello board, int num, double C){//UCBに基づいて手を探すメソッド,定数測定用
        Node n = node[num];
        int select = -1;
        double maxUcb;// = Double.MIN_VALUE;
        maxUcb = -9999;
    ...
    public static int selectBestUct(Othello board, double C){//選ばれた手の中で一番選ばれた回数が多い手を返すメソッド,定数測定用
        nodeNum = 0;
        int next = createNode(board);
        int uctLoop = 10000;
        Othello copy = new Othello();
